...
import { Trophy, Medal, Award, Flame, Clock, CheckCircle } from "lucide-react";
            ...
            <div className="max-w-3xl mx-auto space-y-8">
                ...
                <div className="text-center space-y-4 mb-12">
                    <h1 className="text-4xl md:text-6xl font-black bg-clip-text text-transparent bg-gradient-to-r from-yellow-400 via-orange-500 to-red-600 animate-in fade-in slide-in-from-top-4 duration-700">
                        {votingOpen ? "VOTINGBOARD" : "LEADERBOARD"}
                    </h1>
                        ...
                        `}
                            >
                                <div className="w-12 text-center font-bold text-xl flex justify-center">
                                    {votingOpen ? (
                                        // When voting is open: show cup icons for top 3
                                        isFirst ? <Trophy className="w-8 h-8 text-yellow-500" /> :
                                        isSecond ? <Trophy className="w-8 h-8 text-gray-400 dark:text-slate-400" /> :
                                        isThird ? <Trophy className="w-8 h-8 text-amber-700 dark:text-orange-600" /> :
                                        null
                                    ) : (
                                        // When voting is closed: show checkmark icon based on milestones
                                        rank.completedCount >= 7 ? (
                                            <CheckCircle className="w-8 h-8 text-yellow-500 dark:text-amber-500" />
                                ...
                                <div className="text-right">
                                    ...
                                    {votingOpen && rank.voteCount !== undefined && (
                                        <>
                                            <div className="text-2xl font-black text-foreground leading-none mt-2">
                                                {rank.voteCount}
                                            </div>
